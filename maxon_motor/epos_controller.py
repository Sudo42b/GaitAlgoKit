...
class EPOSController:
    ...
    def open_device(self):
        ...
        # 위치 프로파일 값 설정
        if not self.epos.VCS_SetPositionProfile(self.key_handle, self.node_id, 100, 1000, 1000, byref(self.error_code)):
            self.show_error_information(self.error_code.value)
            return False
    ...
    def update_status(self, gui=None):
        """상태 업데이트"""
        if not self.update_active or self.key_handle == 0:  # 핸들 검사 추가
            return False
        # 변수 명시적 초기화
        mode = c_byte(0)  # 명시적 초기화
        try:
            # 작동 모드 확인
            mode = c_byte()
            if not self.epos.VCS_GetOperationMode(self.key_handle, self.node_id, byref(mode), byref(self.error_code)):
    ...
    def restore_epos(self):
        ...
        if self.initialization and self.key_handle:
            try:
